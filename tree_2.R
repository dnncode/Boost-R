# Tree growing algorithm for Boost-R-D.R
...
tree = function(m.hat.tree, gamma, lambda, data.tree, D.max, z.tree, u, v){
  ...
  for (i in 1:N.data){
    tmp = Tmp.mcf.list[[i]]
    tmp.time = c(tmp$time, ceiling(max(data.tree[i,])))
    
    m[i, 1:(tmp.time[1]-1)] = 0
    if (nrow(tmp)>0){
      for (j in 2:length(tmp.time)){
        m[i, (tmp.time[j-1]):(tmp.time[j]-1)] = j-1
      }
      m[i,tmp.time[j]] = m[i,(tmp.time[j]-1)]  
    }else{
      m[i,] = 0/0
    }
  }
  ...
  h = array(2, dim=c(nrow(g), ncol(g)))
  para.opt = optim(par=rep(0.01,5*ncol(z.tree[[1]])), node.2,
                   g.in = g, h.in = h, z.in = z.tree, gamma.in= gamma, u.in=u, v.in=v,
                   method = c("BFGS"),
  ...
  while (flag==0){
        ...
        for (j in 1:n.x){
          ...
          dummy = foreach(jj = search.range) %dopar% {
            ...
            if ( (nrow(data.left)>n.min) & (nrow(data.right)>n.min) ){
              ...
              z.tree.left = z.tree[data.id[case.left]]
              m =  array(0/0, dim=c(N.data, T.max))
              ...
              for (i in 1:N.data){
                ...
                tmp = Tmp.mcf.list[[i]]
                tmp.time = c(tmp$time, ceiling(max(data.left[i,])))
              ...
              para.opt.left.list  = optim(par=rep(0.01,5*ncol(z.tree[[1]])), node.2,
                                          g.in = g, h.in = h, z.in = z.tree.left,gamma.in= gamma, 
                                          method = c("BFGS"),
                                          control = list(trace=6), hessian = FALSE)
              ...
              T.max = min(T.max, ncol(m.hat.tree[data.id[-case.left],]))
              M.hat.node = m.hat.tree[data.id[-case.left],1:T.max]
              z.tree.right = z.tree[data.id[-case.left]]
              m =  array(0/0, dim=c(N.data, T.max))
              ...
              for (i in 1:N.data){
                #data.tmp = data.input[i, ]
                tmp = Tmp.mcf.list[[i]]
                tmp.time = c(tmp$time, ceiling(max(data.right[i,])))
              ...
              para.opt.right.list  = optim(par=rep(0.01,5*ncol(z.tree[[1]])), node.2,
                                           g.in = g, h.in = h, z.in = z.tree.right ,gamma.in= gamma, 
                                           method = c("BFGS"),
                                           control = list(trace=6), hessian = FALSE)
    ...
    for (i in 1:D){
      if (node.vector[i]==0){
        para.opt.left.list[[i]] = matrix( output.after.list[[i]][4: (4+length(para.optim.left)-1)], ncol=1 )
        para.opt.right.list[[i]] = matrix( output.after.list[[i]][(4+length(para.optim.left)):(4+2*length(para.optim.left)-1)], ncol=1 )
        para.opt.list[[i]] = cbind(para.opt.left.list[[i]], para.opt.right.list[[i]])
        node.individual[count2:(count2+1)] = output.after.list[[i]][(4+2*length(para.optim.left)):(4+2*length(para.optim.left)+1)]
        count2 = count2 + 2
        ...
        # update data list
        data.id.left.after =  output.after.list[[i]][(4+2*length(para.optim.left)+2):length(output.after.list[[i]])]
        tmp = match(data.id.list[[i]],output.after.list[[i]][(4+2*length(para.optim.left)+2):length(output.after.list[[i]])])
